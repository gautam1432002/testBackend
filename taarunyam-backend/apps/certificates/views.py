...
class AdminCertificateUploadView(APIView):
    """
    Receives a frontend-generated PDF (html2canvas) and saves it to the Certificate model.
    This allows 100% visual parity between UI and Email.
    """
    permission_classes = [IsAdmin]
    ...
    def post(self, request):
        ...
        cert_type = request.data.get('type', 'participation')
        pdf_file = request.FILES.get('file')

        if not registration_id or not pdf_file:
            return Response({'success': False, 'message': 'registration_id and file are required.'}, status=400)

        try:
            reg = Registration.objects.select_related('participant', 'event').get(registration_id=registration_id)
        except Registration.DoesNotExist:
            return Response({'success': False, 'message': f'Registration {registration_id} not found.'}, status=404)
        ...
        # Update type and issued_by if already exists
        cert.type = cert_type
        cert.issued_by = request.user

        # Save the uploaded file
        filename = f'certificates/{reg.event.slug}/{cert.qr_token}_fe.pdf'
        if cert.pdf_path:
            cert.pdf_path.delete(save=False)

        cert.pdf_path.save(filename, ContentFile(pdf_file.read()), save=True)

        return _success(CertificateSerializer(cert).data, 'Certificate PDF uploaded successfully.')
